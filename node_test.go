...
func TestNodeFoldedScalarRoundtripStable(t *testing.T) {
	cases := []struct {
		...
		{
			// The exact reproduction from issue #337: a folded line, an empty
			// separator, then a more-indented block.
			name: "folded heading then more-indented block",
			src:  "key: >\n  Heading:\n\n    * first item\n    * second item\n",
		},
		{
			// A single newline between two plain folded lines is preserved by a
			// blank line on emit; this is the case the fix-up break is for.
